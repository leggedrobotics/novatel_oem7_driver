...
bool RosbagRangeDataProcessorRos::validateTopicsInRosbag(const rosbag::Bag& bag,
                                                         const std::vector<std::string>& mandatoryTopics)
{
  ...
  {
    ROS_ERROR(
        "All required topics are not within the rosbag. Replay operations are terminated. Waiting user to terminate.");
    return false;
  }
...
bool RosbagRangeDataProcessorRos::associateAndWriteOdometryMsgs()
{
  ...
  while (!odometryQuque_.empty() && !INSPVAQueue_.empty())
  {
    ...
    while (seqOdomMsg != seqINSVPAMsg)
    {
      ...
      {
        ROS_ERROR_STREAM("ROS odomMsg Seq number going back. Previous " << prevseqOdomMsg_
                                                                        << " AND seqOdomMsg: " << seqOdomMsg);
        odometryQuque_.pop();
      ...
      {
        ROS_ERROR_STREAM("ROS INSPVAQueue_ Seq number going back. Previous " << prevseqINSVPAMsg_
                                                                             << " AND seqINSVPAMsg: " << seqINSVPAMsg);
        INSPVAQueue_.pop();
      ...
      {
        // ROS_WARN_STREAM("ROS IMU Seq number is bigger. seqOdomMsg " << seqOdomMsg << " AND seqINSVPAMsg: " <<
        // seqINSVPAMsg);
        INSPVAQueue_.pop();
        inspvaMsg = INSPVAQueue_.front();
        seqINSVPAMsg = inspvaMsg.header.seq;
      ...
      else
      {
        ...
        // ROS_WARN_STREAM("seqINSVPAMsg number is bigger. seqINSVPAMsg " << seqINSVPAMsg << " AND seqOdomMsg: " <<
        // seqOdomMsg);
        odometryQuque_.pop();
        odomMsg = odometryQuque_.front();
        seqOdomMsg = odomMsg.header.seq;
    ...
    ////////////////////////////////////////////////////////////////////////////
    std::chrono::time_point<std::chrono::steady_clock, std::chrono::nanoseconds> receivedTimeinspvaMsg(
        std::chrono::nanoseconds{ inspvaMsgtoNSEC });
    auto receivedTimeinspvaMsgMilliseconds =
        std::chrono::time_point_cast<std::chrono::milliseconds>(receivedTimeinspvaMsg).time_since_epoch();

    std::chrono::time_point<std::chrono::steady_clock, std::chrono::nanoseconds> receivedTimeodomMsg(
        std::chrono::nanoseconds{ odomMsgtoNSEC });
    auto receivedTimeodomMsgMilliseconds =
        std::chrono::time_point_cast<std::chrono::milliseconds>(receivedTimeodomMsg).time_since_epoch();
    ...
    if (std::fabs(receivedRosTimeDifference.toSec() * 1000) > allowedReceivedTimeDifference)
    {
      ...
      std::cout << std::setprecision(15) << "TimeDiff: " << std::fabs(receivedRosTimeDifference.toSec() * 1000)
                << " ms." << std::endl;
      ...
      if (receivedTimeodomMsgMilliseconds.count() > receivedTimeinspvaMsgMilliseconds.count())
      {
        ...
        {
          std::cout << "Odom msg is ahead of inspva msg. We need to pop the inspva msg. Time Diff: "
                    << std::setprecision(15) << differenceBetweenConsecutiveRosIMUMsgs.toSec() * 1000 << std::endl;
          skipOdometry_ = true;
        }
      }
    ...
    std::chrono::time_point<std::chrono::steady_clock, std::chrono::milliseconds> timePoint(
        std::chrono::milliseconds{ gpsTimeMilli });
    ...
    {
      ROS_ERROR_STREAM(
          "BIG DIFFERENCE BETWEEN CONVERTED ROS TIME AND ORIGINAL RECEIVED TIME. INSVPA TIME CONVERSION MIGHT BE "
          "FAULTY : "
          << conversionDiff.toSec() * 1000);
    }
...
bool RosbagRangeDataProcessorRos::associateAndWriteGPSmsgs()
{
  ...
  while (!rosFixQueue_.empty() && !gpsCommonQuque_.empty())
  {
    ...
    {
      ROS_WARN_STREAM("Seq Numbers are not equal. seqNavsatFix: " << seqNavsatFix
                                                                  << " AND seqGpsCommon: " << seqGpsCommon);

      if (seqNavsatFix < prevSeqNavsatFix_)
      {
        ROS_ERROR_STREAM("ROS navsatFixMsg Seq number going back. Previous " << prevSeqNavsatFix_
                                                                             << " AND seqNavsatFix: " << seqNavsatFix);
        rosFixQueue_.pop();
      ...
      {
        ROS_ERROR_STREAM("ROS gpsCommonMsg Seq number going back. Previous " << prevSeqGpsCommon_
                                                                             << " AND seqGpsCommon: " << seqGpsCommon);
        gpsCommonQuque_.pop();
    ...
    std::chrono::time_point<std::chrono::steady_clock, std::chrono::nanoseconds> receivedTimegpsCommonMsg(
        std::chrono::nanoseconds{ gpsCommonMsgtoNSEC });
    auto receivedTimegpsCommonMsgMilliseconds =
        std::chrono::time_point_cast<std::chrono::milliseconds>(receivedTimegpsCommonMsg).time_since_epoch();

    std::chrono::time_point<std::chrono::steady_clock, std::chrono::nanoseconds> receivedTimenavsatFixMsg(
        std::chrono::nanoseconds{ navsatFixMsgtoNSEC });
    auto receivedTimenavsatFixMsgMilliseconds =
        std::chrono::time_point_cast<std::chrono::milliseconds>(receivedTimenavsatFixMsg).time_since_epoch();

    if (std::fabs(receivedRosTimeDifference.toSec() * 1000) > allowedReceivedTimeDifference)
    {
      ROS_WARN("Time difference between the two messages is too large. Checking association.");
      // The time difference is this
      std::cout << std::setprecision(15) << "Diff: " << std::fabs(receivedRosTimeDifference.toSec() * 1000)
                << std::endl;
      ...
      // If this happens, we know that the rosmsgs is ahead of corr imu and we need to hold the value of rosmsgs such
      // that we can find a match of corrimu.
      if (receivedTimenavsatFixMsgMilliseconds.count() > receivedTimegpsCommonMsgMilliseconds.count())
      {
        ...
        {
          std::cout << "NAVSATFIX IS AHEAD. Time Diff: " << std::setprecision(15)
                    << differenceBetweenConsecutiveGPSFixMsgs.toSec() * 1000 << std::endl;
          skipGpsFix_ = true;
        }
      }
      ...
      if (receivedTimegpsCommonMsgMilliseconds.count() > receivedTimenavsatFixMsgMilliseconds.count())
      {
        ...
        {
          // INSPVA msg is ahead of odom msg. We need to pop the odom msg.
          std::cout << "GPSCOMMON msg is ahead. Time Diff: " << std::setprecision(15)
                    << differenceBetweenConsecutiveGPSCOMMONMsgs.toSec() * 1000 << std::endl;
    ...
    std::chrono::time_point<std::chrono::steady_clock, std::chrono::nanoseconds> timePoint(
        std::chrono::nanoseconds{ gpsTimeMilliNSEC });
    auto seconds = std::chrono::time_point_cast<std::chrono::seconds>(timePoint).time_since_epoch();
    auto nanoseconds = std::chrono::time_point_cast<std::chrono::nanoseconds>(timePoint).time_since_epoch() - seconds;
...
bool RosbagRangeDataProcessorRos::associateAndWriteIMUmsgs()
{
  ...
  while (!corrIMUquque_.empty() && !rosIMUQueue_.empty())
  {
    ...
    while (seqRosIMU != seqCorrIMU)
    {
      ...
      {
        ROS_ERROR_STREAM("ROS IMU Seq number going back. Previous " << prevSeqRosIMU_
                                                                    << " AND seqRosIMU: " << seqRosIMU);
        rosIMUQueue_.pop();
      ...
      {
        ROS_ERROR_STREAM("ROS CORRIMU Seq number going back. Previous " << prevSeqCorrIMU_
                                                                        << " AND seqCorrIMU: " << seqCorrIMU);
        corrIMUquque_.pop();
      ...
      {
        // ROS_WARN_STREAM("ROS IMU Seq number is bigger. seqRosIMU " << seqRosIMU << " AND seqCorrIMU: " <<
        // seqCorrIMU);
        corrIMUquque_.pop();
        corrIMU = corrIMUquque_.front();
    ...
    std::chrono::time_point<std::chrono::steady_clock, std::chrono::nanoseconds> receivedTimeRosImu(
        std::chrono::nanoseconds{ rosIMUtoNSEC });
    auto receivedTimeRosImuMilliseconds =
        std::chrono::time_point_cast<std::chrono::milliseconds>(receivedTimeRosImu).time_since_epoch();

    std::chrono::time_point<std::chrono::steady_clock, std::chrono::nanoseconds> receivedTimeCorrIMU(
        std::chrono::nanoseconds{ corrIMUtoNSEC });
    auto receivedTimeCorrIMUMilliseconds =
        std::chrono::time_point_cast<std::chrono::milliseconds>(receivedTimeCorrIMU).time_since_epoch();

    // Check if we roughlty received the msgs in similar times. This is only subject to communication delays. 100hz ~
    // 10ms between msgs.
    if (std::fabs(receivedRosTimeDifference.toSec() * 1000) > allowedReceivedTimeDifference)
    {
      ROS_WARN("Time difference between the two messages is too large. Will try to associate messages.");
      // The time difference is this
      std::cout << std::setprecision(15) << "Diff: " << std::fabs(receivedRosTimeDifference.toSec() * 1000) << " ms."
                << std::endl;
      ...
      if (receivedTimeCorrIMUMilliseconds.count() > receivedTimeRosImuMilliseconds.count())
      {
        ...
        {
          // INSPVA msg is ahead of odom msg. We need to pop the odom msg.
          std::cout << "CorrIMU msg is ahead of ros IMU msg. We need to pop the odom msg. Time Diff: "
                    << std::setprecision(15) << differenceBetweenConsecutiveRosCorrIMUMsgs.toSec() * 1000 << std::endl;
    ...
    auto nanosecondsOut =
        std::chrono::time_point_cast<std::chrono::nanoseconds>(timePoint).time_since_epoch() - secondsOut;
    ...
    {
      ROS_ERROR_STREAM(
          "BIG DIFFERENCE BETWEEN CONVERTED ROS TIME AND ORIGINAL RECEIVED TIME. GPS TIME CONVERSION MIGHT BE FAULTY : "
          << conversionDiff.toSec() * 1000);
    }
